  ...
  <div id="page">
      ...
      >
        ...
        <el-table-column prop="type" label="类型" min-width="90">
          <template #default="scope">
            ...
            <el-tag type="warning" v-if="scope.row.type[0] === '2'"
              >付费</el-tag
            >
          </template>
        </el-table-column>
        <el-table-column prop="method" label="方法" min-width="80">
          <template #default="scope">
            <el-tag type="success" v-if="scope.row.method === 'get'">{{
              scope.row.method
            }}</el-tag>
            <el-tag type="danger" v-if="scope.row.method === 'post'">{{
              scope.row.method
            }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="110">
          <template #default="scope">
            <el-button
              @click="editAPIButton(scope.row)"
              type="text"
              size="small"
              >修改</el-button
            >
            <el-button @click="deleteAPI(scope.row)" type="text" size="small"
              >删除
            </el-button>
            <el-button @click="onlineDebug(scope.row)" type="text" size="small"
              >在线调试</el-button
            >
          </template>
        </el-table-column>
    ...
    <el-dialog title="添加" :visible.sync="addFormVisible">
      ...
      >
        ...
        <el-form-item label="URL" prop="url">
          <el-input
            v-model="addForm.url"
            placeholder="例：/api/queryIpAddress"
          ></el-input>
        </el-form-item>
        <el-form-item label="请求参数示例" prop="requestParams">
          <el-input
            v-model="addForm.requestParams"
            placeholder="例：cityName='北京'"
          ></el-input>
        </el-form-item>

        <el-form-item label="请求参数类型" prop="requestHeader">
          <el-select
            v-model="addForm.requestHeader"
        ...
        <el-form-item label="响应结果类型" prop="responseHeader">
          <el-select
            v-model="addForm.responseHeader"
            placeholder="请选择请求参数类型"
            style="width: 202px"
          >
            <el-option
        ...
        <el-form-item label="类型" prop="type">
          ...
          >
            ...
            <el-option label="付费" value="20"></el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="修改" :visible.sync="updateFormVisible">
      ...
      >
        ...
        <el-form-item label="请求参数示例" prop="requestParams">
          <el-input
            v-model="updateForm.requestParams"
            placeholder="例：cityName='北京'"
          ></el-input>
        </el-form-item>
        <el-form-item label="请求参数类型" prop="requestHeader">
          <el-select
            v-model="updateForm.requestHeader"
            placeholder="请选择请求参数类型"
            style="width: 202px"
          >
            <el-option
        ...
        <el-form-item label="响应结果类型" prop="responseHeader">
          <el-select
            v-model="updateForm.responseHeader"
            placeholder="请选择请求参数类型"
            style="width: 202px"
          >
            <el-option
...
import {
  ...
  addAPI,
  updateAPI,
} from "@/api/interfaceManagement/APIMessage";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        requestParams: [
          { required: true, message: "请输入请求参数示例", trigger: "blur" },
        ],
        requestHeader: [
  ...
  methods: {
    ...
    //获取类型数值
    getValue(type, method, requestHeader) {
      if (type[0] === "1") {
        if (method === "get" && requestHeader === "application/json") {
          return "11";
        } else if (method === "post" && requestHeader === "application/json") {
          return "12";
        } else if (
          method === "post" &&
          requestHeader === "application/form-data"
        ) {
          return "13";
        }
      } else if (type[0] === "2") {
    ...
    async addAPI(addForm) {
      this.$refs[addForm].validate(async (valid) => {
        if (valid) {
          try {
            this.addForm.type = this.getValue(
              this.addForm.type,
              this.addForm.method,
              this.addForm.requestHeader
            );
    ...
    editAPIButton(row) {
      ...
      this.updateForm = row;
      this.updateForm.type=this.updateForm.type[0]
    },

    //表格修改按钮————保存按钮
    async updateAPI() {
      this.updateForm.type = this.getValue(
        this.updateForm.type,
        this.updateForm.method,
        this.updateForm.requestHeader
      );
